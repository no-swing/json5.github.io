...
+ function($) {
	...
	function clearMenus2() {
	    $(backdrop).remove()
	    $(toggle).each(function(e) {
	        var $parent = getParent($(this))
	        if(!$parent.hasClass('mouseopen')) return
	        $parent.trigger(e = $.Event('hide.' + zuiname))
	        if(e.isDefaultPrevented()) return
	        $parent.removeClass('mouseopen').trigger('hidden.' + zuiname)
	    })
	}
    ...
    $(document)
		...
		.on('mouseenter', '.dropdown', Dropdown.prototype.toggle)
		.on('mouseleave', '.dropdown', Dropdown.prototype.toggle)
}(window.jQuery);
